...
class Node:
    ...
    def __repr__(self) -> str:
        return f"Node #{self.id} ({self.x},{self.y}) f={self.f}, g={self.g}, safe_interval={self.interval}"
...
class Move:
    # start moving from id1 to id2 during interval (t1, t2)
...
class CBS_Tree:
    ...
    def get_paths(self, node, size):
        ...
        while node.parent is not None:
            ...
            if p is None or len(p.nodes) == 0:
                paths[node.paths[0].agentID] = node.paths
            node = node.parent

        for i in range(len(node.paths)):
            if paths[i] is None or len(paths[i].nodes) == 0:
