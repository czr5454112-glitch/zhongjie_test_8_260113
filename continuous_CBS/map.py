...
class Map:
    ...
    def get_valid_moves(self, id, backwards=False):
        if backwards:
            ...
            return [Node(nn, _x=self.nodes[nn].x, _y=self.nodes[nn].y) for nn in self.nodes if
                    (id in self.nodes[nn].neighbours)]
    ...
    def get_heuristic_dist(self, agent, node1, goal_node):
        if self.heuristic is None:
            ...
            if goal_node.id == agent.goal_id:
                ...
                # This approach can overestimate heuristic distances
                ...
                for ag_id in range(len(self.heuristic.h_values[0])):
                    h = max(h, abs(self.heuristic.get_dist(ag_id, node1.id) - self.heuristic.get_dist(ag_id,
                                                                                                      goal_node.id)))
                return h
