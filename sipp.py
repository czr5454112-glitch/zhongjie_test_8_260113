...
class SIPP:
    ...
    def find_successors(self, cur_node: Node, goal: Node):
        ...
        for move in valid_moves:
            ...
            if colls_it:
                # Convert collision intervals into safe intervals
            ...
            # Get the list of constraints for cur_node to new_node
        ...
        for i, node in enumerate(self.open):
            if node.f > new_node.f + self.CN_EPSILON:  # if new_node.f has a lower f-value
                ...
                return
                    ...
                    node.f - new_node.f) < self.CN_EPSILON and new_node.g + self.CN_EPSILON > node.g:  # if f-values are equal, compare g-values
                self.open.append(new_node)
                return
        self.open.append(new_node)
        ...
        if not self.collision_intervals.get(node_id):
            ...
            while k < len(self.collision_intervals[node_id]):
                ...
                while i < len(nodes):
                    ...
                    elif c[0] - self.CN_EPSILON < n.interval[0] < c[1]:
                    ...
                    elif c[0] < n.interval[1] < c[1] + self.CN_EPSILON:
                        nodes[i].interval[1] = c[0]
                        changed = True
    ...
    def find_path(self, agent, cons):
        ...
        if not self.landmarks:
            ...
            if self.verbose:
                print(
                    f"SIPP for [{agent.id}]: {agent.start_id} -> {[f'{m.id1}-{m.id2}' for m in self.landmarks]} -> {agent.goal_id}   | Constraints: {self.constraints}")
            ...
            for i in range(len(self.landmarks) + 1):
                ...
                for k in range(len(parts)):
                    for j in range(len(results)):
                        if parts[k].nodes and \
                                abs(parts[k].nodes[0].interval[0] - results[j].nodes[-1].interval[
                                    0]) < self.CN_EPSILON and \
                                ((math.isinf(parts[k].nodes[0].interval[1]) and math.isinf(
                                    results[j].nodes[-1].interval[1])) or
                                 (abs(parts[k].nodes[0].interval[1] - results[j].nodes[-1].interval[
                                     1]) < self.CN_EPSILON)):
                ...
                if i < len(self.landmarks):
                    ...
                    goals = self.get_endpoints(self.landmarks[i].id2, self.landmarks[i].t1 + offset,
                                               self.landmarks[i].t2 + offset)
...
if __name__ == "__main__":
    ...
    path = planner.find_path(Agent(35, 85, 0), cs)  # Plan path between nodes 35 and 85 for agent 0
